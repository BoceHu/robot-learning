...
class simple_cnn_2(nn.Module):
    def __init__(self):
        super(simple_cnn_2, self).__init__()
...
class RGBBCRobot2(RobotPolicy):
    ...
    def train(self, data):
        ...
        self.CNN = simple_cnn_2()
        # self.optimizer = Adam(self.CNN.parameters(), lr=0.005)
        optimizer = SGD(self.CNN.parameters(), lr=0.03)
        batchsize = 32
        epochs = 32
        train_set = position(X, Y)
        train_loader = DataLoader(dataset=train_set, batch_size=batchsize, shuffle=True, num_workers=4)
